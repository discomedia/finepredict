...
- Set `TEST_DATABASE_URL` to a separate disposable Postgres database before `pnpm test:integration` to run the database-backed product-store suite. Never point it at production.
...
- Better Auth account access requires both `DATABASE_URL` and `BETTER_AUTH_SECRET`; Resend magic links additionally require `RESEND_API_KEY` and a verified `RESEND_FROM_EMAIL`. Keep account routes disabled by leaving `BETTER_AUTH_SECRET` unset when those resources are not configured.
- `ADMIN_EMAILS` is a comma-separated bootstrap allowlist applied when Better Auth first creates a user. Normal settings and dispute administration use an authenticated `admin` session; `ADMIN_API_KEY` is emergency automation only and must not be requested by the browser UI.
...
- Developer API keys are HMAC-hashed with `API_KEY_HASH_SECRET`, which must contain at least 32 characters. `DEVELOPER_API_DAILY_LIMIT` is the per-key atomic daily usage limit.
...
- `MONITOR_DRY_RUN` defaults to `true`. A dry run may persist monitor runs, observations, snapshots, and deduplicated alerts, but it must not send Resend emails or Stripe meter events. Build before running `pnpm --filter @finepredict/api monitor`; use `pnpm --filter @finepredict/api monitor:dev` for a TypeScript local run.
