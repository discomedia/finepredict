...
- Better Auth account access requires both `DATABASE_URL` and `BETTER_AUTH_SECRET`; Disco Mail magic links additionally require `DISCO_MAIL_API_KEY` and a sender on the verified `fp.discomedia.co` domain. FinePredict uses `disco.mail.send` from `@discomedia/utils` for both magic links and monitor alerts. Keep account routes disabled by leaving `BETTER_AUTH_SECRET` unset when those resources are not configured.
