...
- `apps/extension` builds the Manifest V3 browser extension into `apps/extension/dist`; `pnpm --filter @finepredict/extension package` creates the ignored Chrome Web Store zip there. It prefills one supported market URL from Polymarket or Kalshi, or both URLs from supported Kalshi/Polymarket rows on Oddpool's `/arb-dashboard`. It must never submit an analysis without an explicit user action. Keep Chrome Web Store credentials outside the extension bundle and repository.
