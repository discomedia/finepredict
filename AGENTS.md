...
- The public arbitrage scanner runs inside the API when `DATABASE_URL` and `ARBITRAGE_SERVICE_ENABLED=true` are present. Full deterministic Polymarket/Kalshi catalog discovery runs on startup and hourly; a bounded refresh reprices at most 24 persisted pairs every five minutes. It uses no LLM and places no trades. Keep Polymarket order-book reads batched through `POST /books` (up to 500 tokens per request), cache venue fee schedules for 24 hours, and preserve the content-hash write suppression in `arbitrage_markets`. A warm 24-pair refresh should require no more than 25 book requests plus cached fee lookups. Verify storage efficiency with the arbitrage repository integration test, then verify `/api/arbitrage/status`, `/api/arbitrage/opportunities`, and the responsive `/arbitrage` page.
